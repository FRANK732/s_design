...
/// Matches Ant Design's nested options structure.
class SSelectItem<T> {
  const SSelectItem({
    this.value,
    this.label,
    ...
    this.options,
    this.groupLabel,
  }) : assert(options != null || (value != null && label != null), 
          'Either options must be provided (for a group) or both value and label must be provided (for an item).');
  ...
  final List<SSelectItem<T>>? options;
