...
class SScaffold
  ...
  SScaffold({
    ...
    @Deprecated(
        'Use refreshConfig instead')
    this.refreshIndicatorTriggerMode =
        RefreshIndicatorTriggerMode.onEdge,
    @Deprecated(
        'Use refreshConfig instead')
    this.minimumRefreshDuration =
        1000,
    LoadingConfig?
        loadingConfig,
    @Deprecated(
    ...
    this.bodyShimmer,
        ...
        loadingConfig = loadingConfig ??
            (isLoading || loadingIndicator != null || bodyShimmer != null
                ...
                : null),
        assert(
          (loadingConfig?.bodyShimmer ?? bodyShimmer) == null || (loadingConfig?.indicator?.loaderType ?? loadingIndicator?.loaderType) == SLoaderType.shimmer,
          'Shimmer should only be used with shimmer loader type.',
        ),
        assert(
          !(refreshConfig?.enabled ?? enableRefresh) || (refreshConfig?.onRefresh ?? onRefresh) != null,
        ...
        assert(
          !((persistentFooterConfig?.buttons ?? persistentFooterButtons) != null && renderFooter != null),
        ...
        assert(
          !(drawer != null && !drawerEnableOpenDragGesture && onDrawerChanged != null),
          'onDrawerChanged is unnecessary when drawerEnableOpenDragGesture is false',
        ),
        assert(
          !(endDrawer != null && !endDrawerEnableOpenDragGesture && onEndDrawerChanged != null),
  ...
  /// The decoration to apply to the persistent footer.
  /// If null, no decoration is applied to the persistent footer area.
  /// Typically used with [persistentFooterButtons] to customize the footer appearance.
  final BoxDecoration?
      persistentFooterDecoration;
  ...
  /// Builder for the scrim displayed behind a bottom sheet.
  /// If null, uses the default bottom sheet scrim.
  /// Takes BuildContext and animation progress (0.0 to 1.0) and returns a Widget.
  ...
  /// Whether the drawer can be dismissed by tapping on the scrim.
  /// Defaults to true. When true, tapping outside the drawer will close it.
  /// When false, the drawer can only be closed programmatically or by swiping.
  final bool
      drawerBarrierDismissible;
...
class _SScaffoldState
    extends State<
        SScaffold> {
  late LoadingProvider
      _loadingProvider;
  bool
      _isRefreshing =
      false;
  ...
  void
    ...
    _loadingProvider =
        LoadingProvider();
    _loadingProvider.setLoading(widget.loadingConfig?.isLoading ??
        widget.isLoading);
  }
  ...
  Future<void>
    ...
    try {
      await Future.wait(<Future<void>>[
        ...
        Future<void>.delayed(Duration(milliseconds: widget.refreshConfig?.minimumDuration ?? widget.minimumRefreshDuration)),
      ]);
    } catch (e, stackTrace) {
      ...
      if (mounted) {
        setState(() {
          _isRefreshing = false;
          _loadingProvider.setLoading(widget.loadingConfig?.isLoading ?? widget.isLoading);
        });
      }
    }
  }
      ...
      child:
        ...
        children: <Widget>[
          Scaffold(
            ...
            floatingActionButton: widget.floatingActionButtonConfig?.floatingActionButton ?? widget.floatingActionButton,
            floatingActionButtonLocation: widget.floatingActionButtonConfig?.location ?? widget.floatingActionButtonLocation,
            floatingActionButtonAnimator: widget.floatingActionButtonConfig?.animator ?? widget.floatingActionButtonAnimator,
            persistentFooterButtons: widget.persistentFooterConfig?.buttons ?? widget.persistentFooterButtons,
            persistentFooterAlignment: widget.persistentFooterConfig?.alignment ?? widget.persistentFooterAlignment,
            ...
            body: _buildBody(context, widget.loadingConfig?.indicator ?? widget.loadingIndicator),
            bottomNavigationBar: widget.renderFooter != null ? _buildFooter(context) : widget.bottomNavigationBar,
          ),
          if ((widget.loadingConfig?.indicator?.loaderType ?? widget.loadingIndicator?.loaderType) != SLoaderType.shimmer)
            Consumer<LoadingProvider>(
              builder: (BuildContext context, LoadingProvider loadingProvider, Widget? child) {
                return loadingProvider.isLoading && !_isRefreshing ? (widget.loadingConfig?.indicator ?? widget.loadingIndicator ?? _buildLoadingIndicator()) : const SizedBox.shrink();
              },
            ),
        ],
      ),
  ...
  Widget _buildBody(
      ...
      SLoadingIndicator?
          loadingIndicator) {
    final Consumer<LoadingProvider>
        bodyContent =
        Consumer<LoadingProvider>(
      builder: (BuildContext context,
          LoadingProvider loadingProvider,
          Widget? child) {
        if (loadingProvider.isLoading && (widget.loadingConfig?.indicator?.loaderType ?? widget.loadingIndicator?.loaderType) == SLoaderType.shimmer && !_isRefreshing) {
          return (widget.loadingConfig?.bodyShimmer ?? widget.bodyShimmer) ??
              Shimmer.fromColors(
                baseColor: const Color.fromARGB(255, 118, 111, 111),
                highlightColor: Colors.grey[100]!,
                child: widget.renderBody != null ? widget.renderBody!(context) : _buildShimmerLoader(),
              );
        }
        return widget.renderBody != null ? widget.renderBody!(context) : const SizedBox.shrink();
      },
    );
    ...
    final Widget safeContent = widget.useSafeArea
        ? SafeArea(child: paddedContent)
        : paddedContent;

    final Widget content = widget.centerBody
  ...
  Widget _buildFooter(
    ...
    final Widget?
        footerWidget =
        widget.renderFooter?.call(context);
    final double
        additionalBottomPadding =
        MediaQuery.viewPaddingOf(context).bottom;

    if (footerWidget
        is BottomNavigationBar) {
