...
class SchriftFlowLogo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget logoIcon = CustomPaint(
      size: Size(size, size),
      painter: _SchriftFlowLogoPainter(),
    );
        ...
        ShaderMask(
          ...
          child: Text(
            ...
            style: TextStyle(
              ...
              fontSize: size * 0.75,
              color: Colors.white, // Required to show the gradient mask
              letterSpacing: -0.5,
              height: 1.0,
            ),
          ),
...
class _SchriftFlowLogoPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    // The original SVG was drawn on a 32x32 canvas viewBox.
    // We scale the canvas so the paths draw correctly no matter the requested widget size.
    final double scale = size.width / 32;
