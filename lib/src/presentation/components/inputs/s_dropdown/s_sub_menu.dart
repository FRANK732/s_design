...
///
/// Use inside [SMenu] anywhere you need cascading sub-menus, mirroring
/// Ant Design's `SubMenu` component.
///
/// ```dart
/// SMenu(
///   children: [
...
///       children: [
///         SMenuItem(child: const Text('Export'), onTap: () {}),
///         SMenuItem(child: const Text('Archive'), onTap: () {}),
///       ],
///     ),
///   ],
/// )
/// ```
class SSubMenu extends StatelessWidget {
  const SSubMenu({
    super.key,
    required this.title,
    ...
    this.icon,
    this.disabled = false,
    this.placement = STriggerPlacement.rightTop,
    this.popupOffset = const Offset(4, 0),
  });
  ...
  final STriggerPlacement placement;
  ...
  Widget build(BuildContext context) {
    final SThemeData theme = STheme.of(context);

    final Color textColor =
        disabled ? theme.colorToken.textSecondary.withOpacity(0.5) : theme.colorToken.textPrimary;
    final Color iconColor =
        disabled ? theme.colorToken.textSecondary.withOpacity(0.3) : theme.colorToken.textSecondary;

    final Widget popup = Material(
      elevation: 8,
      shadowColor: theme.colorToken.shadow.withOpacity(0.15),
    ...
    return STrigger(
      action: const <STriggerAction>[STriggerAction.hover],
      placement: placement,
      disabled: disabled,
      mouseEnterDelay: const Duration(milliseconds: 80),
      mouseLeaveDelay: const Duration(milliseconds: 120),
      popup: popup,
      child: _SSubMenuRow(
        title: title,
        icon: icon,
...
class _SSubMenuRow extends StatefulWidget {
  const _SSubMenuRow({
    required this.title,
  ...
  @override
  State<_SSubMenuRow> createState() => _SSubMenuRowState();
}

class _SSubMenuRowState extends State<_SSubMenuRow> {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        child: DefaultTextStyle(
          style:
              widget.theme.typographyToken.bodyMedium.copyWith(color: widget.textColor),
          child: IconTheme(
...
/// A split-button dropdown that mirrors Ant Design's `Dropdown.Button`.
///
...
class SDropdownButton extends StatelessWidget {
  const SDropdownButton({
    ...
    this.placement = SDropdownPlacement.bottomRight,
    this.trigger = const <SDropdownTrigger>[SDropdownTrigger.click],
    this.disabled = false,
    this.open,
    this.onOpenChange,
  ...
  final SDropdownPlacement placement;

  /// Trigger modes for the arrow button.
  final List<SDropdownTrigger> trigger;
  ...
  Widget build(BuildContext context) {
    ...
    // Radius splits: left button has left corners, arrow has right corners
    const Radius outerRadius = Radius.circular(6);
    const Radius innerRadius = Radius.circular(0);
